...
class PS3_Controller:
  ...
  def update_axis(self):
    if self.joystick is not None:
      try:
        ...
        print("Left axis : " + str(self.left_axis))
        print("Raw angle : " + str(rawAngle))
        self.right_axis =[ self.joystick.get_axis(2), self.joystick.get_axis(3)]
        print("Right axis : " + str(self.right_axis))
        print("R2 : " + str(self.joystick.get_axis(13)))
      except pygame.error:
        print ("Axis Error")
  ...
  def check_if_connected(self):
    #TODO: find a way to detect whether the controller is connected or not
    return True
    try:
      busses = usb.busses()
      for bus in busses:
  ...
  def run(self, display=False):
    ...
    if display:
        for figure in figures:
          if self.is_pressed(figure):
            if figures[figure][0] == 'circle':
              pygame.draw.circle(self.surface, (255, 0, 0), coords[figure], figures[figure][1], 0)
            elif figures[figure][0] == 'polygon':
              pygame.draw.polygon(self.surface, (255, 0, 0), coords[figure], 0)
            elif figures[figure][0] == 'rectangle':
              pygame.draw.rect(self.surface, (255, 0, 0), coords[figure], 0)
        pygame.display.update()
    self.left_axis = [self.joystick.get_axis(0), self.joystick.get_axis(1)]
    return self.left_axis, self.joystick.get_axis(13), self.running
